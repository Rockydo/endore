...
6. PAUSED M5 renderer: isolate or replace the inherited `boat_with_oars_unit` path that
...
7. PAUSED M5 evidence: run a fresh non-debug five-year Observer economy session and
   capture population, market, settlement, raw-material, and development map modes.
8. PAUSED M5 roads: pursue a materially different Arda-native `spline_network.splnet` route
   (binary-format writer or editor-safe source overlay); do not repeat the two exhausted
   retail map-editor launches.
9. PAUSED M5 gate: install the validated 302-edge route graph only with matching native splines,
   then repeat paired smoke and the five-year economy evidence run.
10. Later map tooling: revisit Himling/Tolfalas adjacency candidates only through a bounded
