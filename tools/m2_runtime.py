...
def pops_payload(owned: list[str]) -> str:
    if M3_REALMS.is_file():
        ...
        from m3_realms import build_state
...
def start_payloads() -> dict[Path, str]:
    ...
    files = {
        ...
        "15_international_organizations.txt": (
            ...
            f"\t\tmembers = {{ {'GON' if M3_REALMS.is_file() else 'SWE'} }}\n"
            f"\t\tleader = {'GON' if M3_REALMS.is_file() else 'SWE'}\n"
            f"\t\temperor = {{ {'GON' if M3_REALMS.is_file() else 'SWE'} }}\n"
            "\t}\n"
            "}\n"
        ),
...
def other_payloads() -> dict[Path, str]:
    ...
    for path in sorted(installed_country_setup.glob("*.txt")):
        ...
        if path.name == "_scandinavia.txt" and M3_REALMS.is_file():
            continue
