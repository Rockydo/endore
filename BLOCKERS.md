...
## 2026-08-02 - Native lake-mesh carriers remain rectangular

Status: two-strike renderer-route blocker; experiment removed, material ponds retained.

The installed `lake_mesh`/`lake_layer` ABI is addressable on the custom world through the
exact retail `lakes_locators.txt` filename and a headerless 40-byte transform bin. Ten
Arda-only transforms loaded twice and rendered animated water without a parser or loading
failure. They did not, however, conform to the source ponds. At terrain-relative offset
0.42 with 1.24 carrier margin, and again at offset 0.10 with 1.08 margin, the unit-square
mesh remained visibly rectangular around Mirrormere/Khazad-dum rather than being clipped
to the shallow authored basin.

Do not run a third offset/scale variant. The native mesh is a square carrier and this
build does not depth-clip it tightly enough to create a cartographically faithful small
lake. The exact definition and 400-byte experimental bin were removed; the pushed v75
wet-material ponds and complete Earth-object quarantine were regenerated byte-clean.
Future work needs a genuinely polygonal water mechanism, not another carrier offset.
Evidence: `docs/screens/docs/screens/20260802_v76_native_ponds/` and
`docs/screens/20260802_v76b_native_ponds/`.

## 2026-08-02 - Broad retail green-scenery families fail Arda close zoom

Status: renderer-route blocker; palm and grass layers quarantined, sparse rocks retained.

Three bounded palm populations (26,000, 4,000, then 1,600 transforms) and a final
zero-palm fresh-game A/B left the repeated green rosette carpet around Qarsad and Umbar
visually unchanged. A separate zero-grass A/B removed it completely. The installed
`vegetation_grass_test_mesh` family therefore reads as generic shrubs at player camera
scales even when every transform is clipped to an authored climate polygon. The sparse
palm population produced no distinct, defensible visual benefit in the same evidence.

Do not tune either density again. Their exact retail definition filenames are
quarantined, their experimental bins are removed, and validation rejects stale palm or
grass outputs. A future southern-vegetation pass requires a materially different installed
mesh or bespoke referenced art, not another transform-count adjustment. Evidence:
`docs/screens/20260802_v77_climate_scenery/`,
